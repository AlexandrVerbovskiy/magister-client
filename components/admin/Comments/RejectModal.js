...
const RejectModal = ({ active, close, onAcceptClick }) => {
  ...
  const handleRejectAcceptClick = async () => {
    ...
    if (checkedList.length < 1) {
      setCheckedListError(
        "You must specify the reasons for rejecting the review"
      );
      return;
    }
    ...
    if (checkedList.includes("other") && declineOtherDescription.length < 1) {
      setDeclineOtherDescriptionError(
        "You must enter the reason for the rejection of the review"
      );
      return;
    }
        ...
        <div style={{ width: "100%" }}>
          <div className="mb-2">
            <div className="text-lg font-semibold text-slate-800 dark:text-slate-100">
              Are you sure you want reject this review?
            </div>
          </div>
